  ...
  \item
    ...
    \begin{enumerate}
      ...
      \item We may write the expectation value as:
        ...
        Since $X$ and $Y$ have a mean of 0, we get:

        $$\boxed{E[W]=0}$$

        The autocorrelation becomes:

        $$R_{WW}(t,\tau)=E[W(t)W(t+\tau)]$$
        $$R_{WW}(t,\tau)=E[(X(t)\cos(\omega_ot)+Y(t)\sin(\omega_ot))(X(t+\tau)\cos(\omega_o(t+\tau))$$$$+Y(t+\tau)\sin(\omega_o(t+\tau)))]$$

        We further expand to get:

        $$R_{WW}(t,\tau)=R(\tau)\cos(\omega_ot)\cos(\omega_o(t+\tau))+R(\tau)\sin(\omega_ot)\sin(\omega_o(t+\tau))$$
        ...
        $$R_{WW}(t,\tau)=R(\tau)[\cos(\omeg_ot-\omega_o(t+\tau)])$$
        $$\boxed{R_{WW}(t,\tau)=R(\tau)\cos(\omega_o\tau)}$$

        Thus, because the mean is constant and the autocorrelation does not depend on $t$, we conclude that $W$ \underline{is wide-sense stationary}

      \item We can use the autocorrelation function of $X$ to find the spectral density:

        $$S_XX(f)=\int_0^{\infty}R(\tau)\cos(2\pi f\tau)\,d\tau$$
        $$S_XX(f)=25\int_0^{\infty}\cos(2\pi f\tau)e^{-20000\tau^2}\,d\tau$$

        We continue to evaluate:

        $$S_XX(f)=12.5\int_0^{\infty}e^{-10000\pi^2f^2\tau^2}\,d\tau$$

        We take $u\to 100\pi f\tau$ to get:

        $$S_XX(f)=\frac{1}{8\pi f}\int_0^{\infty}e^{-u^2}\,du$$
        $$S_XX(f)=\frac{1}{8\pi f}\left[ \frac{\sqrt{\pi}}{2} \right]$$
        $$\boxed{S_XX(f)=\frac{1}{16\sqrt{\pi} f}}$$

        We find the power over the frequency range by writing:

        $$P_X=\int_0^{100} \frac{1}{16\sqrt{\pi}f}\,df$$
        $$P_X=\frac{1}{16\sqrt{\pi}}(\ln(100))$$
        $$\boxed{P_X=.1624}$$
